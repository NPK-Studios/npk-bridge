...
Bridge.Setup.Point = function()
    ...
    AddTextEntry("NPKBridgeHelpNotification", 'Press ~INPUT_ATTACK~ to select point\nPress ~INPUT_VEH_DUCK~ to cancel')
    local currentCoords = nil
    Bridge.Setup.inSetup = true
    Citizen.CreateThread(function()
        while Bridge.Setup.inSetup do
            Citizen.Wait(0)
            local hit, entityHit, coords, _, _ = Bridge.libs.raycast.fromCamera(511, 4, 15.0)
    ...
    Citizen.CreateThread(function()
        while Bridge.Setup.inSetup do
            ...
            if currentCoords then
                DisplayHelpTextThisFrame("NPKBridgeHelpNotification", false)
                DrawMarker(
                ...
                if IsControlPressed(0, 24) or IsDisabledControlPressed(0, 24) then
                    Bridge.Setup.inSetup = false
                    SetClipboardText(('vec3(%.2f, %.2f, %.2f),'):format(currentCoords.x, currentCoords.y, currentCoords.z))
...
Bridge.Setup.Ped = function()
    ...
    local pedModel = Bridge.libs.requestModel('mp_m_freemode_01')
    ...
    Citizen.CreateThread(function()
        while Bridge.Setup.inSetup do
            ...
            if currentCoords then
                ...
                if IsControlPressed(0, 24) or IsDisabledControlPressed(0, 24) then
                    Bridge.Setup.inSetup = false
                    SetClipboardText(('vec4(%.2f, %.2f, %.2f, %.2f),'):format(currentCoords.x, currentCoords.y, currentCoords.z + 1.0, GetEntityHeading(currentPed)))
        ...
        while Bridge.Setup.inSetup do
            ...
            if currentCoords then
                ...
                if IsControlPressed(0, 24) or IsDisabledControlPressed(0, 24) then
                    ...
                    SetClipboardText(('vec3(%.2f, %.2f, %.2f), vec3(%.2f, %.2f, %.2f)'):format(currentCoords.x, currentCoords.y, currentCoords.z, rotation.x, rotation.y, rotation.z))
...
RegisterNetEvent('npk-bridge/client/setup/start', function(setupType)
