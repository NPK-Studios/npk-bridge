...
RegisterNetEvent('QBCore:Player:SetPlayerData', function(xPlayer) 
    TriggerEvent('npk-bridge/client/setPlayerData', xPlayer)
end)
...
AddEventHandler('onClientResourceStart', function(resourceName)
    ...
    Citizen.Wait(1000)
    TriggerEvent('npk-bridge/client/setPlayerData', playerData)
end)
...
Bridge.Framework.getIdentifier = function()
    ...
    if not identifier then
        Bridge.libs.print.error('QB-Core PlayerData not loaded yet. Please wait for the playerLoaded event.')
        return nil
    end
...
Bridge.Framework.getPlayerName = function()
    ...
    if not playerData then
        Bridge.libs.print.error('Player data is not available.')
        return nil
    end
...
Bridge.Framework.CheckJobDuty = function()
    ...
    local onDuty = exports['qbx_core']:GetPlayerData()?.job?.onduty
    return onDuty == true
end