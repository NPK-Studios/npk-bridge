...
if Config.Debug then
    Bridge.libs.print.info('[Target] Loaded: textui')
end
...
--@param parameters: table [coords: vector3, size?: vector3 | { x|length, y|width, z|height }, options: table]
Bridge.Target.addBoxZone = function(parameters)
    local size = parameters.size
    local length, width
    if type(size) == 'vector3' or (type(size) == 'table' and size.x) then
        length, width = size.x or size[1], size.y or size[2]
    elseif type(size) == 'table' then
        length, width = size.length or size[1], size.width or size[2]
    end
    ...
    local point = lib.points.new({
        coords = parameters.coords,
        distance = math.max(length, width) + 1.0,
    })
