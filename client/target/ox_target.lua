...
if Config.Debug then
    Bridge.libs.print.info('[Target] Loaded: ox_target')
end
...
--@param parameters: table [coords: vector3, size?: vector3 | { x|length, y|width, z|height }, rotation?: number (heading), debug?: boolean, options: table]
Bridge.Target.addBoxZone = function(parameters)
    local resourceName = GetInvokingResource() or cache.resource
    local size = parameters.size
    if type(size) == 'table' and not size.x then
        size = vec3((size.length or size[1] or 1.0) + 0.0, (size.width or size[2] or 1.0) + 0.0, (size.height or size[3] or 2.0) + 0.0)
    end

    local targetId = exports['ox_target']:addBoxZone({
        coords = parameters.coords,
        size = size or vec3(1.0, 1.0, 2.0),
        rotation = parameters.rotation or parameters.heading or 0.0,
        debug = parameters.debug or Config.Debug or false,
        options = parameters.options,
    })

    for i = 1, #parameters.options, 1 do
        Bridge.Target.targets[resourceName] = Bridge.Target.targets[resourceName] or {}
        table.insert(Bridge.Target.targets[resourceName], {type = 'boxZone', id = targetId})
    end
    return targetId
end
...
AddEventHandler('onClientResourceStop', function(resourceName)
    if Bridge.Target.targets[resourceName] then
        for _, target in pairs(Bridge.Target.targets[resourceName]) do
            ...
            elseif target.type == 'boxZone' then
                Bridge.Target.removeBoxZone(target.id)
