...
if Config.Debug then
    Bridge.libs.print.info('[Appearance] Loaded: tgiann-clothing')
end

Bridge.Appearance = {}

Bridge.Appearance.fetchCurrentSkin = function()
    local currentSkin = Bridge.Callback.await('npk-bridge/server/getPlayerSkin')

    if Config.Debug then
        Bridge.libs.print.info('[Appearance] Fetched current skin:', currentSkin)
    end
...
Bridge.Appearance.fetchDatabaseSkin = function()
    local databaseSkin = Bridge.Callback.await('npk-bridge/server/getPlayerSkin')

    if Config.Debug then
        Bridge.libs.print.info('[Appearance] Fetched database skin:', databaseSkin)
    end
...
Bridge.Appearance.convertSkinFormat = function(skinData)
    if not skinData or type(skinData) ~= 'table' then
        Bridge.libs.print.error('[Appearance] Skin data is nil or not a table!')
        return
    end
...
Bridge.Appearance.setPlayerSkin = function(skinData)
    if not skinData then
        Bridge.libs.print.error('[Appearance] Skin data is nil or empty!')
        return
    end
    ...
    if Config.Debug then
        Bridge.libs.print.info('[Appearance] Set player skin:', skinData)
    end
end

Bridge.Appearance.setPlayerClothing = function(clothingData)
    if not clothingData then
        Bridge.libs.print.error('[Appearance] Clothing data is nil or empty!')
        return
    end
    ...
    if Config.Debug then
        Bridge.libs.print.info('[Appearance] Set player clothing:', clothingData)
    end
end
