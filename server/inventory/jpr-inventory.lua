...
if Config.Debug then
    Bridge.libs.print.info('[Inventory] Loaded: jpr-inventory')
end

Bridge.Inventory = {}

RegisterNetEvent('npk-bridge/inventory/openInventory', function(invType, data)
...
Bridge.Inventory.CustomDrop = function(prefix, items, coords)
    Bridge.libs.print.error('CustomDrop is not supported in jpr-inventory, please change type in config')
end
...
Bridge.Inventory.getItemCount = function(playerId, itemName, itemMetadata)
    if itemMetadata and QBCore then
        ...
        for k, v in pairs(items) do
            if v.name == itemName and v.info and Bridge.libs.table.matches(v.info, itemMetadata) then
                return v.amount
            end
        end
    else
