if (Config.Dispatch == 'auto' and not checkResource('0r-dispatch')) or (Config.Dispatch ~= 'auto' and Config.Dispatch ~= '0r-dispatch') then
    return
end
...
if Config.Debug then
    Bridge.libs.print.info('[Dispatch] Loaded: 0r-dispatch')
end
...
Bridge.Dispatch.SendAlert = function(playerId, data)
    ...
    exports['0r-dispatch']:SendAlert(
        ...
        data.title,
        data.code,
        data.icon or "fa-dollar-sign",
        data.job or {'police'},
        data.blip?.sprite
    )
end

RegisterNetEvent('npk-bridge/server/dispatch/sendAlert', function(data)
