-- Server-side mirror of client/locale/main.lua.
-- Same API: Bridge.Locale.load(resource, lang) / .t(resource, key, ...) / .create(resource) / .has / .getDict

while not Bridge do Wait(0) end

if Config.Debug then
    if Bridge.libs and Bridge.libs.print and Bridge.libs.print.info then
        Bridge.libs.print.info('[Locale] Loaded (server)')
    else
        print('[npk-bridge] [Locale] Loaded (server)')
    end
end
