...
if Config.Debug then
    Bridge.libs.print.info('[Framework] Loaded: QBOX')
end

RegisterNetEvent('QBCore:Server:PlayerLoaded', function(playerId)
    TriggerEvent('npk-bridge/server/playerLoaded', playerId) -- DONT TOUCH IT!
end)
...
Bridge.Framework.SetOfflineJob = function(uniqueId, jobName, jobGrade)
    ...
    local jobsData = exports['qbx_core']:GetJobs()
    row.job = json.decode(row.job)
    local newJob = {
        name = jobName,
        label = jobsData[jobName]?.label or 'Unknown',
        onduty = row.job.onduty or false,
        type = jobsData[jobName]?.type or 'none',
        grade = {
            name = jobsData[jobName]?.grades[tostring(jobGrade)]?.name or 'No Grades',
            level = jobGrade,
            payment = jobsData[jobName]?.grades[tostring(jobGrade)]?.payment or 0,
            isboss = jobsData[jobName]?.grades[tostring(jobGrade)]?.isboss or false
        }
    }
...
Bridge.Framework.getPlayerById = function(playerId)
    if not playerId then
        if Config.Debug then
            Bridge.libs.print.error('Player ID is required to fetch player data.')
        end
        return nil
    end
    ...
    if not xPlayer then
        if Config.Debug then
            Bridge.libs.print.error(('No player found with ID: %s'):format(playerId))
        end
        return nil
    end
...
Bridge.Framework.getPlayerId = function(uniqueId)
    if not uniqueId then
        if Config.Debug then
            Bridge.libs.print.error('Unique ID is required to fetch player ID.')
        end
        return nil
    end
...
Bridge.Framework.getPlayerByUniqueId = function(uniqueId)
    if not uniqueId then
        if Config.Debug then
            Bridge.libs.print.error('Unique ID is required to fetch player data.')
        end
        return nil
    end
    ...
    if not xPlayer then
        if Config.Debug then
            Bridge.libs.print.error(('No player found with unique ID: %s'):format(uniqueId))
        end
        return nil
    end
...
Bridge.Framework.getUniqueId = function(playerId)
    if not playerId then
        if Config.Debug then
            Bridge.libs.print.error('Player ID is required to fetch unique ID.')
        end
        return nil
    end
...
Bridge.Callback.register('npk-bridge/server/framework/checkPermissions', Bridge.Framework.checkPermissions)
