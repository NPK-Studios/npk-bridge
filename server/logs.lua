...
Bridge.Logs.Send = function(playerId, title, message, webhook)
    Citizen.CreateThread(function()
        ...
        if not message or not webhook then 
            Bridge.libs.print.info('[ERROR] No message or webhook provided for logging [' .. (GetInvokingResource() or GetCurrentResourceName()) .. ']')
            return 
        end
        ...
        local embedData = { {
            ...
            ['color'] = 14423100,
            ['footer'] = {
                ['text'] = ("%s | NPKStudios | " .. os.date()):format(title or GetInvokingResource()),
                ['icon_url'] = "https://www.npkteam.pl/api/uploads/a12fbe1c-f88f-4933-af47-e8ef0168b1c2.png"
            },
            ['description'] = message,
            ['author'] = {
                ['name'] = "NPKStudios",
