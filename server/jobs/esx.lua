...
if Config.Debug then
    Bridge.libs.print.info('[Jobs] Loaded: ESX')
end
...
local function resolveUniqueId(player)
    ...
    return Bridge.Framework.getUniqueId(player)
end
...
Bridge.Jobs.refresh = function()
    ...
    local grades = MySQL.query.await('SELECT job_name, grade, name, label, salary FROM job_grades') or {}

    local out = {}
    for _, job in ipairs(jobs) do
        out[job.name] = { name = job.name, label = job.label or job.name, grades = {} }
    end

    for _, grade in ipairs(grades) do
        local job = out[grade.job_name]
        if not job then
            job = { name = grade.job_name, label = grade.job_name, grades = {} }
            out[grade.job_name] = job
        end
        job.grades[tostring(grade.grade)] = {
            grade = tonumber(grade.grade) or 0,
            name = grade.name or ('grade' .. tostring(grade.grade)),
            label = grade.label or tostring(grade.grade),
...
Bridge.Jobs.getStored = function(uniqueId)
    local stored = decode(MySQL.scalar.await('SELECT jobs FROM users WHERE identifier = ?', { uniqueId }), {})
...
Bridge.Jobs.setStored = function(uniqueId, list)
    MySQL.update.await('UPDATE users SET jobs = ? WHERE identifier = ?',
...
Bridge.Jobs.getPlayerJobs = function(player)
    ...
    local dutytimes = decode(MySQL.scalar.await('SELECT dutytime FROM users WHERE identifier = ?', { uniqueId }), {}) or {}
    local jobs = {}
...
Bridge.Jobs.setGrade = function(player, jobName, grade)
    ...
    elseif not playerId then
        local offline = Bridge.Framework.getOfflinePlayerByUniqueId(uniqueId)
        if offline and offline.job == jobName then
            Bridge.Framework.SetOfflineJob(uniqueId, jobName, grade)
        end
    end
...
--@param jobName: string
--@return table[] [{ identifier, name, grade, online }]
Bridge.Jobs.getEmployees = function(jobName)
    local rows = MySQL.query.await([[
        SELECT identifier, firstname, lastname, jobs FROM users
    ...
    for _, src in ipairs(GetPlayers()) do
        local uniqueId = Bridge.Framework.getUniqueId(tonumber(src))
    ...
    for _, row in ipairs(rows) do
        ...
        out[#out + 1] = {
            ...
            name = (((row.firstname or '') .. ' ' .. (row.lastname or '')):gsub('^%s+', ''):gsub('%s+$', '')),
            grade = grade,
            online = online[row.identifier] ~= nil,
            source = online[row.identifier],
        }
    end
...
CreateThread(function()
    while not Bridge.Framework do Wait(100) end
    Bridge.Jobs.refresh()
end)
