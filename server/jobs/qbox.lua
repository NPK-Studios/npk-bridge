...
if Config.Debug then
    Bridge.libs.print.info('[Jobs] Loaded: Qbox')
end
...
Bridge.Jobs.refresh = function()
    local shared = exports['qbx_core']:GetJobs() or {}
    local out = {}
...
Bridge.Jobs.create = function(name, label, grades)
    ...
    for _, grade in ipairs(grades or {}) do
        definition.grades[tonumber(grade.grade) or 0] = {
    ...
    local ok = pcall(function()
        exports['qbx_core']:CreateJobs({ [name] = definition })
    end)
